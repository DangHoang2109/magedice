...
public class PageView : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
{
    ...
    public CallbackEventInt changePageEvent = new CallbackEventInt();

    public CallbackEventInt changePageEventCompleted = new CallbackEventInt();
    ...
    public void _Start()
    {
        //this._scrollRectComponent.content.localPosition = new Vector3(this._scrollRectComponent.content.sizeDelta.x / 2, this._scrollRectComponent.content.localPosition.y, this._scrollRectComponent.content.localPosition.z);
        ...
        SetPage(this.startingPage);
        InitPageSelection();

        SetPageSelection(this.startingPage);
    ...
    private void OnDisable()
    {
        ...
        if (prevButton)
            prevButton.GetComponent<Button>().onClick.RemoveListener(() => { PreviousScreen(); });
    }
    ...
    private void ScaleMidlePage()
    {
        for (int i = 0; i < this._scrollRectComponent.content.childCount; i++)
        {
            this._scrollRectComponent.content.GetChild(i).localScale = Vector3.one;
        }
        Transform obj = this._scrollRectComponent.content.GetChild(this._currentPage);
        if (obj != null)
        {
            obj.DOScale(this.scaleCenter, 0.1f);
        }

        //event done change page view
        changePageEventCompleted?.Invoke(this._currentPage);
    }

    //------------------------------------------------------------------------
    public void LerpToPage(int aPageIndex, bool onCallback= true)
    {
        if (this._pagePositions.Count <= 0)
        {
            return;
        ...
        _lerp = true;
        bool isScale = false;
        if (this._currentPage != aPageIndex)
        {
            isScale = true;
        }
        _currentPage = aPageIndex;

        if (onCallback) this.changePageEvent?.Invoke(aPageIndex);
    ...
    public void SetPageSelection(int aPageIndex)
    {
        ...
        if (_pageSelectionImages != null)
        {
            ...
            this.changePageEvent?.Invoke(aPageIndex);
        }
    ...
    {
        if (this._currentPage >= this._scrollRectComponent.content.childCount - 1)
        {
